...
public class RunIterator implements Iterator {
  ...
  // the encoding.
  RunIterator(DListNode node,DList runDList) {
	  currentNode=node;
	  this.runDList=runDList;
  ...
  public int[] next() {
    ...
    // Replace the following line with your solution.
	  if(hasNext()){
		  ...
		  int[] index = new int[4];
		  index[0] = currentNode.number;
		  index[1] = currentNode.nodePixel.R;
		  index[2] = currentNode.nodePixel.G;
		  index[3] = currentNode.nodePixel.B;
		  return index;
	  }
	  else{
